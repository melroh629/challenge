...
// 사용자 응답 예시 데이터
const userResponses = [2, 3, 4, 3, 4, 1, 3, 4];

// 사용자 응답에 따라 점수 계산
function calculateScore(response, disagreeType, agreeType) {
	switch (response) {
		case 0: // 매우 아니다
			scores[disagreeType] += 2;
			break;
		case 1: // 아니다
			scores[disagreeType] += 1;
			break;
		case 2: // 보통이다
			// 양쪽에 0점
			break;
...
questions.forEach((question, index) => {
	calculateScore(userResponses[index], question.disagree, question.agree);
});
...
function determineMBTI() {
	const result = [
		...
		scores.J >= scores.P ? 'P' : 'J',
	];
	return result.join('');
}

// 결과 출력
console.log('MBTI Result:', determineMBTI());
